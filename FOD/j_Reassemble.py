...
class Resample(nn.Module):
    def __init__(self, hconf, p, s, h, emb_dim, resample_dim):
    ...
    def forward(self, x):
        x = data_parallel(self.conv1, x, self.hconf)  # self.conv1(x)
        x = data_parallel(self.conv2, x, self.hconf)  # self.conv2(x)
        return x


class Reassemble(nn.Module):
    def __init__(self, hconf, image_size, read, p, s, emb_dim, resample_dim):
        """
        p = patch size
        ...
        # Projection + Resample
        self.resample = Resample(hconf, p, s, image_height, emb_dim, resample_dim)
