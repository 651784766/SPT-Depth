...
from FOD.j_Reassemble import Reassemble
from FOD.j_Fusion import Fusion
from FOD.j_Head import HeadDepth, HeadSeg
...
class FocusOnDepth(nn.Module):
    def __init__(
        ...
        """
        p = patch size
        重采样系数s = coefficient resample
        emb层的维度emb_dim <=> D (in the paper)
        再采样维度resample_dim <=> ^D (in the paper)
        读模块类型read : {"ignore", "add", "projection"}
        """
        #emb_dim=768 or 1024
        ...
        # Reassembles Fusion  重组，融合
        #append函数 可以添加如数组，元祖在内数据类型，将他们放在一起
        self.reassembles = []
        self.fusions = []
        for s in reassemble_s:
            self.reassembles.append(
                Reassemble(
                    hconf, image_size, read, patch_size, s, emb_dim, resample_dim
                )
            )
            self.fusions.append(Fusion(hconf, resample_dim))
        ...
        # Head
        if type == "full":
            self.head_depth = HeadDepth(hconf, resample_dim)
            self.head_segmentation = HeadSeg(hconf, resample_dim, nclasses=nclasses)
        elif type == "depth":
            self.head_depth = HeadDepth(hconf, resample_dim)
    ...
    def forward(self, img):
        ...
        t = data_parallel(                                  # noqa: F841      very essential!!!
            self.transformer_encoders, img, self.hconf)     # self.transformer_encoders(img)
        previous_stage = None
